...
static cairo_bool_t
_match_antialias (const cairo_clip_t *clip, cairo_antialias_t antialias)
{
    cairo_clip_path_t *path;

    if (!clip || clip->path == NULL)
	return TRUE;

    path = clip->path;
    while (path) {
	if (path->antialias == antialias)
	    path = path->prev;
	else
	    return FALSE;
    }
...
_clip_to_polygon (const cairo_clip_t *clip, 
		  ...
		  cairo_antialias_t *antialias)
{
    ...
    return _cairo_clip_get_polygon (clip, polygon, fill_rule, antialias);

}

/* intersect path with clip and convert into traps */
static cairo_status_t
_fill_path_clip_intersect_to_traps (const cairo_path_fixed_t *path, 
				    cairo_fill_rule_t   path_fill_rule,
				    double		tolerance,
				    const cairo_clip_t *cached_clip,
				    cairo_polygon_t    *cached_polygon,
				    ...
				    cairo_traps_t      *traps)
{
    ...
    cairo_polygon_t *new_clip_polygon = NULL;
    cairo_polygon_t path_polygon;
    cairo_fill_rule_t fill_rule = path_fill_rule;

    if (clip || cached_clip) {
	//long now = get_tick ();
	_cairo_polygon_init (&path_polygon, NULL, 0);

 	status = _cairo_path_fixed_fill_to_polygon (path, tolerance,
						    &path_polygon);
	if (unlikely (status))
	    goto FINISH;
	//printf ("\tfill_to_polygon %ld\n", get_tick () - now);
        /* if there is a cached polygon */
	if (cached_clip)
	    new_clip_polygon = cached_polygon;
	else if (clip) {
	    //now = get_tick();
	    status = _clip_to_polygon (clip, clip_polygon,
				       clip_fill_rule, clip_antialias);

	    if (unlikely (status))
		goto FINISH;
	    //printf ("\tclip_to_polygon %ld\n", get_tick () - now);
	    new_clip_polygon = clip_polygon;
	}

	if (new_clip_polygon) {
	    //now = get_tick ();
	    status = _cairo_polygon_intersect (&path_polygon,
					       path_fill_rule,
					       new_clip_polygon, 
					       *clip_fill_rule);
	    if (unlikely (status))
...
FINISH:
    if (clip || cached_clip)
	_cairo_polygon_fini (&path_polygon);
...
static cairo_int_status_t
_draw_traps (cairo_xcb_surface_t	*dst,
	     cairo_operator_t	 op,
	     const cairo_pattern_t	*pattern,
	     cairo_traps_t              *traps,
	     int dst_x, int dst_y,
	     ...
	     cairo_antialias_t		 antialias)
{
    ...
    _cairo_xcb_surface_set_precision (dst, antialias);
					...
					cairo_antialias_t             antialias)
{
    cairo_xcb_surface_t *surface = (cairo_xcb_surface_t *) composite->surface;
    cairo_operator_t op = composite->op;
    cairo_pattern_t *source = (cairo_pattern_t *)composite->original_source_pattern;
    cairo_int_status_t status;
    cairo_bool_t match_clip, match_antialias;
    cairo_traps_t traps;
    ...
    long  now = get_tick ();
    match_antialias = _match_antialias (clip, antialias);
    //printf ("match %ld\n", get_tick () - now);
	...
	cairo_antialias_t clip_antialias;
 	cairo_fill_rule_t clip_fill_rule;
        ...
        cairo_bool_t is_region = _cairo_clip_is_region (clip);

        //now = get_tick();
	match_clip = _cairo_clip_equal (surface->clip, clip);
        //printf ("equal %ld\n", get_tick () - now);
	if (match_clip) {
	    status = _fill_path_clip_intersect_to_traps (path, fill_rule,
							 tolerance,
							 surface->clip,
							&surface->clip_polygon,
							 clip,
							 NULL, 
							&clip_fill_rule,
							&clip_antialias,
    ...
    status = _draw_traps (surface, op, source, &traps, 
			  0, 0, &composite->bounded, antialias);
